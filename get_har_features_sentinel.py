...
def evaluate_segment(segment):
    ...
    if (segment.shape[0] * segment.shape[1] > 630) \
...
if __name__ == "__main__":
    ...
    csv_path = 'sentinel/segments/output_sentinel-2_campaign_1_response_time_sorted.csv'
    ...
    for SA in df['Study_Area'].unique():
        ...
        image_path = f'/home/ebneto/datasets/scenes_sentinel_pca/pca_{region}.npy'
        image = np.load(image_path).astype(np.uint8)

        truth_path = f'/home/ebneto/datasets/parsed_masks_sentinel/truth_{region}.npy'
        truth = np.load(truth_path)

        slic_path = f'sentinel/slics/mask_slics_bkp_campaign_1_sentinel-2/mask_slic_{region}.npy'
    ...
    class_info_df.to_csv(f'dataset_har/output_entropy_{sensor}.csv', index=False)
